...
function Stats() {
  ...
  const dummyCupData = 1000;

  const calculateUsage = (dummyCupData * 0.58 * 365) / 9.5;
  const formattedNumber = Math.floor(calculateUsage);

  const treesNeeded = (dummyCupData * 110 * 365) / 25000;
  const formattedTreesNeeded = Math.floor(treesNeeded);
  console.log(`trees needed ${treesNeeded}`);

  const milesEquivalent = dummyCupData * 365 * 0.59;
  const formattedMilesEquivalent = Math.floor(milesEquivalent);

  const factsArray = [formattedNumber, formattedTreesNeeded, treesNeeded];

  const factBankObject = {
    dishwasher: {
      introText: "AND Has Saved Enough Energy To Run A Dishwasher:",
  ...
  //console.log(`factbank random ${factBankObject.dishwasher.introText}`);

  function chooseFact() {
    //console.log(factsArray[Math.floor(Math.random() * 3)]);
        ...
        <div className="max-w-md">
          ...
          <div>
            ...
            <div className="flex justify-evenly items-center">
              <h2 id="dishwasherX">{fact.figure} In A Year!</h2>
              <img id="dishwasherImg" src={fact.img} alt="" />
            </div>
            <p>{fact.src}</p>
          </div>
        </div>
